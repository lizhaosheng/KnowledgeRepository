...
import net.contentobjects.jnotify.JNotifyConstant;
...
public class JNotify_win32
{
	static
	{
		try
		{
			String path = JNotifyConstant.LIB_CLASS_PATH + "/jnotify_win_64.dll";
			System.out.println("try to load '" + path + "' from classpath");
			File lib = Util.getTempLib(path, ".dll");
			System.load(lib.getAbsolutePath());
		}
		catch (UnsatisfiedLinkError e)
		{
			System.err.println("load libjnotify_win_64.dll failed!\n" + e.getMessage());
			try
			{
				String path = JNotifyConstant.LIB_CLASS_PATH + "/jnotify_win.dll";
				System.out.println("try to load '" + path + "' from classpath");
				File lib = Util.getTempLib(path, ".dll");
				System.load(lib.getAbsolutePath());
			}
			...
			{
				System.err.println("load libjnotify_win.dll failed!\n" + e2.getMessage());
				throw e2;
			} catch (IOException e3) {
				System.err.println("failed to get lib file!\n" + e3.getMessage());
				e3.printStackTrace();;
			}
		}
